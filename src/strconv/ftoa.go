...
// FormatFloat 将浮点数转换位字符串，其中 fmt 表示转换后字符串格式，prec 表示浮点数的精度。
// 它假设原始数据是从一个 bitSize 位浮点数获得的（32 表示 float32，64 表示 float64），从获得更加完美的值。
//
// fmt 格式是：
// 'b' (-ddddp±ddd, 二进制指数格式)
// 'e' (-d.dddde±dd, 十进制指数格式),
// 'E' (-d.ddddE±dd, 十进制指数格式),
// 'f' (-ddd.dddd, 非指数格式),
// 'g' (使用 'e' 对应格式表示大指数, 否则使用 'f' 对应格式表示),
// 'G' (使用 'E' 对应格式表示大指数, 否则使用 'f' 对应格式表示),
// 'x' (-0xd.ddddp±ddd, 十六进制分数和二进制指数格式), 或者
// 'X' (-0Xd.ddddP±ddd, 十六进制分数和二进制指数格式)。
//
// 精确度 prec 控制由 'e', 'E', 'f', 'g', 'G', 'x', 和 'X' 打印的数字（不包含指数）。
// 对于 'g' 和 'G'，它是有效数字的最大数量（删除尾部零）。
// 当 prec = -1 时，则代表使用最少数量的、但又必需的数字来表示f。
func FormatFloat(f float64, fmt byte, prec, bitSize int) string {
	return string(genericFtoa(make([]byte, 0, max(prec+4, 24)), f, fmt, prec, bitSize))
}

// AppendFloat 将浮点数 f 使用 FormatFloat 转换为字符串，添加到 dst 中，并返回扩展后的 slice。
